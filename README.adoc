...
[NOTE]
====
The `EmailNotification` parameter is instantiated in the `Controller` using data from the request or/and a data source.
====
...
docker run -d \
  ...
  -e SPRING_OUTBOX_CONNECTOR_RABBIT_MESSAGES_ORDERPLACED_ROUTINGKEY=shopify.orders \
...
For a complete example with multi-module architecture, check out the link:spring-outbox-sample[spring-outbox-sample].
