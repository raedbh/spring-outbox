...
Create the `Order` as `RootEntity`.
+
[source, java]
----
@Entity
...
. **Implement the business logic in the Root Entity**
+ Add a `markPaid` method to the `Order` entity implementing order payment logic and assign the corresponding event:
+
[source, java]
----
@Entity
