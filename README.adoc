...
The high-level architecture for this requirement is as follows:

image::resources/order_placed_diagram.svg[]

Here’s how you can implement this design with Spring Outbox:

. **Add Spring Outbox Dependencies**: add the necessary Spring Outbox dependencies to your Spring Boot application. Here’s an example of the required dependencies when using JPA and RabbitMQ as the message broker.
+
[source, xml]
----
